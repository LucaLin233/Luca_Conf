...
const DEFAULT_CLEAR_KEYS = [
  ...
  "联系", "邮箱", "工单", "USE", "USED", "TOTAL", "EXPIRE", "EMAIL"
];
const REGIONS = [
  {
    "code": "HK",
...
function buildMatchers() {
  ...
  REGIONS.forEach((region, regionIndex) => {
    ...
    [...new Set(aliases.filter(Boolean))].forEach((alias) => {
      matchers.push({ region, regionIndex, alias, isFlag: alias === region.inputFlag, regex: aliasRegExp(alias) });
    });
  });
...
function findRegion(name) {
  let bestText = null, bestFlag = null;
  MATCHERS.forEach((matcher) => {
    ...
    if (!current || index < current.index || (index === current.index && value.length > current.value.length)) {
      const candidate = { ...matcher, index, value };
      if (matcher.isFlag) bestFlag = candidate; else bestText = candidate;
    }
  });
...
function contains(name, keyword) {
  return config.caseSensitive ? name.includes(keyword) : name.toLowerCase().includes(keyword.toLowerCase());
}
function clearReason(name) {
  if (config.clear) {
    const key = DEFAULT_CLEAR_KEYS.concat(config.clearKeys).find((item) => contains(name, item));
    if (key) return "clear:" + key;
  }
  const excluded = config.exclude.find((item) => contains(name, item));
  if (excluded) return "exclude:" + excluded;
  if (config.include.length && !config.include.some((item) => contains(name, item))) return "include:no-match";
  return "";
}
function multiplierValue(name) {
  let match = name.match(/(?:^|[^\d.])(\d+(?:\.\d+)?)\s*(?:x|×|倍)(?:$|[^A-Za-z])/i);
...
function transformProxy(proxy, index) {
  ...
  if (!multiplierAllowed(originalName)) {
    log("DROP " + JSON.stringify(originalName) + " (multiplier:" + config.multiplier + ")"); return null;
  }
  const found = findRegion(originalName);
  if (!found && config.unknown === "drop") { log("DROP " + JSON.stringify(originalName) + " (unknown)"); return null; }
  ...
  log("KEEP " + JSON.stringify(originalName) + " -> " + JSON.stringify(name) +
    (found ? " [" + found.alias + " => " + found.region.code + "]" : " [unknown]"));
...
function sortItems(items) {
  if (config.sort === "region") items.sort((a, b) => a.regionIndex - b.regionIndex || a.index - b.index);
  else if (config.sort === "name") {
    items.sort((a, b) => a.proxy.name.localeCompare(b.proxy.name, "zh-CN") || a.index - b.index);
  }
  return items;
}
